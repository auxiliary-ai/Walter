...
SESSION_HOURS = int(TOTAL_SESSION_HOURS)
TOTAL_CYCLES = (SESSION_HOURS * 3600) // INTERVAL
    ...
    try:
        while True:
            try:
                ...
                logger.info(
                    "News processed: %d major narratives found.", len(major_titles)
                )

                market_snapshot = get_market_snapshot(COIN_TICKER, INTERVAL, HL_URL, 24)
                dashboard.set_state(
                    market_snapshot=market_snapshot,
                    stage="collecting_account",
                )
                logger.info(
                    "Market data fetched. Current Price: $%.2f",
                    market_snapshot.get("current_price", 0),
                )

                account_snapshot = get_open_position_details(HL_URL, GEN_PUB_KEY)
                dashboard.set_state(
                ...
                if decision.action == "close":
                    ...
                    if closed:
                        ...
                        dashboard.add_event(
                            f"CLOSE: {COIN_TICKER} position closed",
                            current_time,
                        )
                        ...
                        dashboard.add_event(
                            "CLOSE: no position to close or failed",
                            current_time,
                        )
                ...
                if (
                    available is None
                    or required_margin > available
                    or order_args["size"] <= 0
                ):
                    available_str = (
                    ...
                    _persist_cycle(
                        ...
                        order_args=order_args,
                        order_placed=False,
                        decision_action_override=(
                            f"{decision.action}_rejected_insufficient_balance"
                        ),
                    )
                    dashboard.set_state(
                        stage="cycle_complete",
                ...
                logger.info(
                    "Placing %s order for %s %s @ %sx lev",
                    decision.action.upper(),
                    fmt_num(order_args["size"], 5),
                    COIN_TICKER,
                    decision.leverage,
                )
                ...
                if order_placed:
                    ...
                    dashboard.add_event(
                        (
                            ...
                            f"{fmt_num(order_args['size'], 5)} {COIN_TICKER} @ lev {decision.leverage}"
                        ),
                        current_time,
                    )
